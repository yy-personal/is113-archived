<?php
    ...
    */
    ...
    // insert each record into "employment" table in database 
    foreach($array_records as $employment) {
        
        // error handling
        if(!isValid($employment)) {
           echo $employment->_id . "has incomplete data";
           continue;
        }

        // add into database
        $add_ok = add($stmt, $employment);
        if(!$add_ok) {
    ...
    function createDBTable() {
        ...
        if(!$isOk) {
            echo "Error: Employment table cannot be created!";
        }
    ...
    function add($stmt, $employment) {

        $year = format($employment->year);
        $university = format($employment->university);
        $school = format($employment->school);
        $degree = format($employment->degree);
        $rate = format($employment->employment_rate_overall);
        $salary = format($employment->basic_monthly_mean);
    ...
    function isValid($employment) {
        if(!isset($employment->year)) {
            return FALSE;
        }
        
        if(!isset($employment->university)) {
            return FALSE;
        }

        if(!isset($employment->school)) {
            return FALSE;
        }
        if(!isset($employment->degree)) {
            return FALSE;
        }
        if(!isset($employment->employment_rate_overall)) {
            return FALSE;
        }
        if(!isset($employment->basic_monthly_mean)) {
            return FALSE;
        }
